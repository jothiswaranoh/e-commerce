...
const features = [
  { icon: Truck, title: 'Free Shipping', description: 'On orders over ₹999', color: 'bg-primary-50 text-primary-500' },
  { icon: Shield, title: 'Secure Payment', description: '100% secure transactions', color: 'bg-emerald-50 text-emerald-500' },
  { icon: TrendingUp, title: 'Best Prices', description: 'Guaranteed low prices', color: 'bg-amber-50 text-amber-500' },
  { icon: Sparkles, title: 'Quality Products', description: 'Verified & authentic', color: 'bg-rose-50 text-rose-500' },
];
        ...
        <div className="relative max-w-[1400px] mx-auto px-6 lg:px-8 py-20 md:py-32">
          <div className="max-w-2xl">
            <div className="inline-flex items-center gap-2 px-3.5 py-1.5 bg-white/10 backdrop-blur-sm border border-white/10 rounded-full mb-7">
              ...
              <span className="text-xs font-bold tracking-wider uppercase text-primary-200">New Arrivals Every Week</span>
            </div>
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              <Link to={ROUTES.PRODUCTS}>
                <button className="inline-flex items-center gap-2 px-7 py-3.5 bg-primary-600 hover:bg-primary-500 text-white text-sm font-bold rounded-xl transition-all shadow-lg hover:-translate-y-0.5">
            ...
            <div className="mt-10 flex items-center gap-5">
              <div className="flex -space-x-2">
                {['bg-primary-400', 'bg-violet-400', 'bg-pink-400', 'bg-amber-400'].map((c, i) => (
      ...
      <section className="relative overflow-hidden bg-gradient-to-br from-neutral-900 via-primary-950 to-neutral-900">
        ...
        <div className="relative max-w-[1400px] mx-auto px-6 lg:px-8 py-20">
          <div className="max-w-xl mx-auto text-center">
            ...
            <p className="text-slate-400 text-lg mb-8">{HOME.sections.promotion.subtitle}</p>
            <Link to={ROUTES.PRODUCTS}>
              <button className="inline-flex items-center gap-2 px-8 py-4 bg-primary-600 hover:bg-primary-500 text-white text-sm font-bold rounded-xl transition-all shadow-lg hover:-translate-y-0.5">
      ...
      <section className="bg-white border-t border-gray-100">
        <div className="max-w-[1400px] mx-auto px-6 lg:px-8 py-16">
          <div className="max-w-2xl mx-auto text-center">
            ...
            <h2 className="text-3xl font-bold text-gray-900 tracking-tight mb-2">{HOME.sections.newsletter.title}</h2>
            <p className="text-gray-400 mb-8">{HOME.sections.newsletter.subtitle}</p>
            ...
            {subscribed ? (
              ...
              <form onSubmit={handleSubscribe} className="flex flex-col sm:flex-row gap-3 max-w-md mx-auto">
                <input
                  ...
                  className="flex-1 h-11 px-4 text-sm bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:border-primary-400 focus:ring-2 focus:ring-primary-100 placeholder:text-gray-300 transition-colors"
                />
                <button type="submit" className="h-11 px-6 bg-primary-600 hover:bg-primary-700 text-white text-sm font-bold rounded-xl transition-colors shadow-sm flex-shrink-0">
                  {HOME.sections.newsletter.primaryCTA}
                </button>
              </form>
            )}
            <p className="mt-4 text-xs text-gray-400">No spam, ever. Unsubscribe at any time.</p>
          </div>
        </div>
      </section>
